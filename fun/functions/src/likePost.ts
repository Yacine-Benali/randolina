...
export const likePost = functions.https.onCall(async (data, context) => {
    ...
    let postLikeCollection: string = `posts/${postId}/likes`;
    // query if user already like the document if its the case return
    let querySnapshot1 = await
        db.collection(postLikeCollection).where('likedBy', 'array-contains', uid).get();
    if (querySnapshot1.docs.length != 0) {
        return;
    }

    // query for the Like document that is not full(<10 000) in the postLikeCollection
    // if it exist add the user in it, check if its full,if it is then mark it as full
    // update post number of likes
    let querySnapshot2 = await
